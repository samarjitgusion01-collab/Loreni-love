...
async function sceneDateChoice(root) {
  ...
  catStage.appendChild(makeCat("cat-bob"));
  root.appendChild(catStage);
  root.appendChild(el("p", { class: "line" }, "If we could go anywhere together"));
  root.appendChild(el("p", { class: "line" }, "right now, where would you choose?"));

  const opts = el("div", { class: "options" });
  root.appendChild(opts);
  const reaction = el("p", { class: "line italic", style: "margin-top:20px;display:none" });
  root.appendChild(reaction);
  const btnRow = el("div", { class: "btn-row" });
  root.appendChild(btnRow);

  const reactions = {
    0: "Salt in the air, your hand in mine. Deal. 🌊",
    1: "Cold outside, warm right next to you. 🏔️",
    2: "New streets, same us. I like that. 🌃",
    3: "Honestly? That sounds perfect too. 🏠",
  };

  DATE_OPTIONS.forEach((d, i) => {
    opts.appendChild(el("button", { class: "option-card", onclick: () => {
      state.dateChoice = d.label;
      saveState();
      opts.querySelectorAll(".option-card").forEach((o) => o.style.opacity = "0.45");
      opts.children[i].style.opacity = "1";
      reaction.textContent = reactions[i];
      reaction.style.display = "block";
      spawnHearts(6);
      if (!btnRow.querySelector(".btn")) {
        btnRow.appendChild(el("button", { class: "btn", onclick: () => next() }, "Continue ❤️"));
      }
    }}, [el("span", { class: "emoji" }, d.emoji), d.label]));
  });
}

// 13. Food date callback
async function sceneFoodDate(root) {
  const catStage = el("div", { class: "cat-stage" });
  catStage.appendChild(makeCat("cat-bob"));
  root.appendChild(catStage);
  const food = state.favoriteFood || "your favorite food";
  root.appendChild(el("p", { class: "line" }, `A date with me + ${food}...`));
  root.appendChild(el("p", { class: "line italic" }, "Honestly, sounds perfect. 👀"));

  const btnRow = el("div", { class: "btn-row" });
  root.appendChild(btnRow);
  btnRow.appendChild(el("button", { class: "btn", onclick: () => next() }, "It really does ❤️"));
}

// 14. Who loves more
async function sceneWhoLoves(root) {
  const catStage = el("div", { class: "cat-stage" });
  catStage.appendChild(makeCat("cat-bob"));
  root.appendChild(catStage);
  root.appendChild(el("p", { class: "line" }, "Be honest..."));
  root.appendChild(el("p", { class: "line italic" }, "Who loves the other more?"));

  const reaction = el("p", { class: "line italic", style: "margin-top:16px;display:none" });
  const btnRow = el("div", { class: "btn-row" });

  function pick(who) {
    state.whoLovesMore = who;
    saveState();
    reaction.textContent = who === "me"
      ? "Wrong answer. It's obviously me. 😤❤️"
      : "...okay fine, maybe it's a tie. 🥹";
    reaction.style.display = "block";
    spawnHearts(6);
    if (!btnRow.querySelector(".btn")) {
      btnRow.appendChild(el("button", { class: "btn", onclick: () => next() }, "Continue ❤️"));
  ...
  const choiceRow = el("div", { class: "btn-row" });
  choiceRow.appendChild(el("button", { class: "btn-ghost", onclick: () => pick("me") }, "Me ❤️"));
  choiceRow.appendChild(el("button", { class: "btn-ghost", onclick: () => pick("you") }, "You ❤️"));
  root.appendChild(choiceRow);
  root.appendChild(reaction);
  root.appendChild(btnRow);
}

// 15. Evil no button (do you love me)
async function sceneDoYouLoveMe(root) {
  const catStage = el("div", { class: "cat-stage" });
  const cat = makeCat("cat-bob");
  catStage.appendChild(cat);
  root.appendChild(catStage);
  const line = el("p", { class: "line" }, "Do you love me? ❤️");
  root.appendChild(line);
  const sub = el("p", { class: "line italic", style: "min-height:1.6em" });
  root.appendChild(sub);
  ...
  let attempts = 0;
  const yesBtn = el("button", { class: "btn", onclick: () => {
    state.doYouLoveMe = "yes";
    saveState();
    cat.classList.remove("cat-bob", "cat-sad");
    cat.classList.add("cat-jump");
    spawnHearts(16);
    fireConfetti();
    sub.textContent = "I KNEW IT 😭❤️";
    noBtn.remove();
    setTimeout(() => next(), 1400);
  }}, "YES ❤️");

  const noBtn = el("button", { class: "btn-ghost", onclick: () => {
    attempts = Math.min(attempts + 1, NO_MESSAGES.length - 1);
    state.noAttempts = attempts;
    saveState();
    sub.textContent = NO_MESSAGES[attempts];
    cat.classList.remove("cat-bob");
    cat.classList.add("cat-sad");
    if (attempts >= MAX_NO_ATTEMPTS - 1) {
...
// 16. Moving no button (stay with me)
async function sceneStayWithMe(root) {
  const catStage = el("div", { class: "cat-stage" });
  const cat = makeCat("cat-bob");
  catStage.appendChild(cat);
  root.appendChild(catStage);
  root.appendChild(el("p", { class: "line" }, "Will you stay with me? ❤️"));
  const sub = el("p", { class: "line italic", style: "min-height:1.6em" });
  root.appendChild(sub);
  ...
  const yesBtn = el("button", { class: "btn", onclick: () => {
    state.stayWithMe = "yes";
    saveState();
    cat.classList.add("cat-jump");
    spawnHearts(14);
    sub.textContent = "Good. That's all I needed to hear. ❤️";
    setTimeout(() => next(), 1300);
  }}, "YES ❤️");

  const noBtn = el("button", { class: "btn-ghost", onclick: (e) => {
    e.preventDefault();
  }}, "NO");

  function fleeButton() {
    chaseCount++;
    if (chaseCount >= CAPTURE_AT) {
      sub.textContent = "THE CAT HAS CAPTURED THE NO BUTTON. 🐈";
      noBtn.disabled = true;
      noBtn.style.opacity = "0.35";
      noBtn.classList.remove("btn-fleeing");
      noBtn.style.position = "";
      cat.classList.add("cat-jump");
      spawnHearts(6);
      return;
    }
    sub.textContent = CHASE_MESSAGES[(chaseCount - 1) % CHASE_MESSAGES.length];
    ...
    noBtn.classList.add("btn-fleeing");
    noBtn.style.left = x + "px";
    noBtn.style.top = y + "px";
  }

  // desktop: flee on hover; touch: flee on touchstart; both: flee before click registers
  ...
  noBtn.addEventListener("click", (e) => {
    if (chaseCount < CAPTURE_AT) { e.preventDefault(); fleeButton(); }
  });
...
// 17. Pet the cat mini game
async function scenePetCat(root) {
  root.appendChild(el("p", { class: "line" }, "Pet the cat ❤️"));
  root.appendChild(el("p", { class: "subtext" }, "Tap him a few times."));

  const petZone = el("div", { class: "cat-stage pet-zone" });
  const cat = makeCat("cat-bob");
  petZone.appendChild(cat);
  root.appendChild(petZone);

  const count = el("div", { class: "pet-count" }, `${state.petTaps || 0} pets so far`);
  root.appendChild(count);

  const spoiledMsg = el("p", { class: "line italic", style: "display:none;margin-top:10px" }, "Okay okay... he's spoiled now.");
  root.appendChild(spoiledMsg);

  const btnRow = el("div", { class: "btn-row" });
  root.appendChild(btnRow);
  const continueBtn = el("button", { class: "btn", onclick: () => next() }, "Continue ❤️");
  btnRow.appendChild(continueBtn);

  petZone.addEventListener("click", (e) => {
    state.petTaps = (state.petTaps || 0) + 1;
    saveState();
    count.textContent = `${state.petTaps} pets so far`;
    cat.classList.remove("cat-bob");
    cat.style.animation = "none";
    void cat.offsetWidth;
    cat.style.animation = "";
    cat.classList.add("cat-jump");
    setTimeout(() => { cat.classList.remove("cat-jump"); cat.classList.add("cat-bob"); }, 600);

    const heart = el("div", { class: "purr-heart" }, "💗");
    const rect = petZone.getBoundingClientRect();
    heart.style.left = (e.clientX ? e.clientX - rect.left : rect.width / 2) + "px";
    heart.style.top = (e.clientY ? e.clientY - rect.top : rect.height / 2) + "px";
    petZone.style.position = "relative";
    petZone.appendChild(heart);
    setTimeout(() => heart.remove(), 900);

    if (state.petTaps >= 6) spoiledMsg.style.display = "block";
  });
}

// 18. Fake loading screen
async function sceneFakeLoading(root) {
  root.appendChild(el("p", { class: "line" }, "Preparing your next question..."));
  const catStage = el("div", { class: "cat-stage" });
  catStage.appendChild(makeCat("cat-sleep"));
  root.appendChild(catStage);
  ...
  const pct = el("p", { class: "subtext" }, "0%");
  root.appendChild(pct);

  const steps = [10, 35, 67, 99, 100];
  ...
  await wait(500);
  root.appendChild(el("p", { class: "line italic" }, "The cat fell asleep."));
  await wait(1000);
  next();
}

// 19. Memory question
async function sceneMemory(root) {
  const catStage = el("div", { class: "cat-stage" });
  catStage.appendChild(makeCat("cat-bob"));
  root.appendChild(catStage);
  root.appendChild(el("p", { class: "line" }, "What's one memory with me that"));
  root.appendChild(el("p", { class: "line" }, "you never want to forget?"));

  const field = el("div", { class: "field" });
  const textarea = el("textarea", { class: "textarea", placeholder: "Tell me..." });
  textarea.value = state.memory || "";
  field.appendChild(textarea);
  root.appendChild(field);

  const cardBox = el("div", { style: "width:100%" });
  ...
  const btn = el("button", { class: "btn", onclick: () => {
    ...
    card.appendChild(el("p", { class: "eyebrow" }, "A memory worth keeping ❤️"));
    card.appendChild(el("p", { class: "memory-text" }, v));
    cardBox.appendChild(card);

    const closing = el("p", { class: "line italic", style: "margin-top:16px" }, "I'll keep this one close too.");
    cardBox.appendChild(closing);
...
// 20. Final section lead-in
async function sceneFinalLeadIn(root) {
  const catStage = el("div", { class: "cat-stage" });
  catStage.appendChild(makeCat("cat-walk-across"));
  root.appendChild(catStage);
  const l1 = el("p", { class: "line" });
  const l2 = el("p", { class: "line" });
  const l3 = el("p", { class: "line" });
  ...
  await typewriter(l1, `Okay ${state.nickname || GIRLFRIEND_NAME}...`, 28);
  await wait(400);
  await typewriter(l2, "We're almost at the end.", 26);
  await wait(500);
  await typewriter(l3, "One last thing.", 26);

  btnRow.appendChild(el("button", { class: "btn", onclick: () => next() }, "Okay ❤️"));
}

// 21. Final question
async function sceneFinalQuestion(root) {
  const catStage = el("div", { class: "cat-stage" });
  const cat = makeCat("cat-bob");
  catStage.appendChild(cat);
  root.appendChild(catStage);
  root.appendChild(el("p", { class: "line" }, "No matter how far apart we are right now..."));
  root.appendChild(el("p", { class: "line italic" }, "Will you keep choosing me? ❤️"));

  const btnRow = el("div", { class: "btn-row" });
  root.appendChild(btnRow);

  const yesBtn = el("button", { class: "btn", onclick: async () => {
    state.finalAnswer = "yes";
    saveState();
    cat.classList.remove("cat-bob");
    cat.classList.add("cat-run-off");
    spawnHearts(20);
    fireConfetti();
    await wait(900);
    submitAnswers();
...
// 22. Final message
async function sceneFinalMessage(root) {
  root.appendChild(el("p", { class: "eyebrow" }, "for you"));
  const card = el("div", { class: "final-message" });
  FINAL_MESSAGE.forEach((p) => card.appendChild(el("p", {}, p)));
  root.appendChild(card);
  root.appendChild(el("p", { class: "signoff" }, "❤️"));
...
// 23. Final cat scene / replay
async function sceneFinalCat(root) {
  const catStage = el("div", { class: "cat-stage" });
  catStage.appendChild(makeCat("cat-bob"));
  root.appendChild(catStage);

  const wrap = el("div", { class: "envelope-wrap" });
  const envelope = el("div", { class: "envelope open" });
  const paper = el("div", { class: "letter-paper" });
  paper.style.height = "128px";
  paper.style.transform = "translateY(-46px)";
  ...
  await wait(900);
  root.appendChild(el("p", { class: "line" }, `For ${GIRLFRIEND_NAME} ❤️`));
  root.appendChild(el("p", { class: "line italic" }, "Made with love."));

  const btnRow = el("div", { class: "btn-row" });
  root.appendChild(btnRow);
  btnRow.appendChild(el("button", { class: "btn", onclick: () => goTo(0) }, "Read it again 🐈"));
}

/* ============================================================
   SUBMIT ANSWERS TO BACKEND (Telegram)
   ============================================================ */
let submitted = false;
async function submitAnswers() {
  ...
  const payload = {
    ...
    favoriteColor: state.favoriteColor ? state.favoriteColor.name : "",
    ...
    choices: {
      ...
      whoLovesMore: state.whoLovesMore,
      doYouLoveMe: state.doYouLoveMe,
      noAttempts: state.noAttempts,
      stayWithMe: state.stayWithMe,
      petTaps: state.petTaps,
  ...
  try {
    ...
    // Fail silently for her — the cat got distracted, but she's still done.
